...
describe Github do
  ...
  it "sets defaults" do
    Github::Configuration.keys.each do |key|
      expect(Github.instance_variable_get(:"@#{key}")).to eq(Github::Default.send(key))
    end
  end
  ...
  describe ".client" do
    ...
    it "responds to keys" do
      ...
      expect(Github.connection_options).to eq({:headers=>{:accept=>"application/vnd.github.v3+json",
